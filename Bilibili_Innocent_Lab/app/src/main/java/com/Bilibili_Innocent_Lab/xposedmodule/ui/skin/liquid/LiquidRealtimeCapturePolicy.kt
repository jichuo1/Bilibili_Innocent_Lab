...
internal object LiquidRealtimeCapturePolicy {
     ...
     * 倍率固定意味着成本随面板分辨率平方增长，而这张图最终要经过 34dp 折射带、8.5dp 内透镜、
     * 5.5dp 散射和 1.8dp 色散的主动扭曲，高频细节本来就被抹掉。
     *
     * 改为预算制后缓冲尺寸与面板分辨率解耦：1080×2400 仍取约 0.71 倍（几乎不变），
     * 1440×3200 降到约 0.53 倍，单缓冲 5.2 MiB、三缓冲 15.6 MiB。
     * [MAX_TARGET_SCALE] 保证低分屏不会被反向放大到超过原有清晰度。
     */
    const val TARGET_SAMPLE_PIXELS = 1_300_000L
