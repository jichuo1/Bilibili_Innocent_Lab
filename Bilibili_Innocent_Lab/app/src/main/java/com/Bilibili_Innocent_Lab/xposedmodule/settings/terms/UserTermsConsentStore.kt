...
/**
 * 模块自身的用户条款状态，不进入 Yuki prefs、设置备份或宿主进程缓存。
 *
...
internal object UserTermsConsentStore {
    ...
    private const val PREF_FILE = "user_terms_consent"
    ...
    fun accept(context: Context): Boolean = write(context, UserTermsDecision.ACCEPTED)

    fun decline(context: Context): Boolean = write(context, UserTermsDecision.DECLINED)
